...
%
% Clicking on the help button will open the "Programming examples" section of
% the plt help file. This is done with the "hh.exe" chm reader utility on
% windows based systems while the browswer is used for other operating systems.
%
% If you are running a very old version of Matlab (older than 7.0) then the
% gui1 button is replaced by the gui1v6 button. gui1.m uses a uitable which
...
case 'help',  % help button callback
  ff = 'pltfiles/Programming with plt/Programming examples.htm'; % help topic
  fc = which('plt.chm');
  if ispc & length(fc) dos(['hh ' fc '::' ff]); % for windows systems, display with chm utility
  else fc = which('plt.htm');
       if length(fc)>6 web(['file:///' fc(1:end-7) '/' ff]); end; % otherwise display topic using the browser
  end;
case {'init','go'}
  fcn = {'plt5'      'afilt'     'bounce(0)' 'circles12' 'curves go' ...
         'dice(0)'   'editz'     'gauss'     'gui1'      'julia'     ...
  ...
  v = version;  if sscanf(v(1:3),'%f')<7 fcn{9} = 'gui1v6'; end; % for Matlab 6
  ...
  bt(end-1) = uicontrol('Pos',[x y 315 18],'user',0,'str','All Demos',...
                      'Enable','inactive','buttond',{@AllDemos,fcn});
  bt(end) = uicontrol('Pos',[x+322 y 37 18],'str','help','callback','demoplt(''help'');');
  set(bt,'units','norm','fontsize',10,'backgr',bbk);
  set(bt(end-1),'backgr',fliplr(bbk)); % use a different color for the All Demos button
  ...
  if isempty(fpos) & sz(4)>1030
    fpos = figpos([1510 5 710 480]); % use a bigger demoplt figure for large screen sizes
  end;
