...
function julia(in1)
  ...
  xy = {[ -3 .093 .110 .395 .84+yi;   -2 .010 .010 .034 .130];  %  left plot positions
        [ -3 .593 .110 .395 .84+yi;   -2 .530 .030 .032 .110;   % right plot positions
          208 .103 .044 .083 .030;    206 .398 .044 .083 .030;  %  left column: x cursor readouts
          207 .103 .007 .083 .030;    205 .398 .007 .083 .030;  % right column: y cursor readouts
          224 .603 .044 .083 .030;    222 .898 .044 .083 .030;
          223 .603 .007 .083 .030;    221 .898 .007 .083 .030]};
  ...
  for n=1:2                                    % create left/right plots
    ...
    S(n).dep = plt('edit',p{4}+q, 0,'callbk',cb,'label','De~pth');
  ...
  text(-.23,-.115,VerString,'units','norm','color',[1 1 1]/3,'fontsize',8);
  ...
  tic; frac(-1); frac(-2);                                    % measure time for 50 iterations
...
function frac(n)   % draw the fractal -----------------------------------------------
  ...
  if n < 0 | exl
    ...
    if n<0 if strcmp(get(S.box,'vis'),'on') plt('cursor',S.cid,'restore'); end; % box off
           if ~f v = version;  v = sscanf(v(1:3),'%f');
                 if v<7  dp = .6*dp; end; % matlab v6 main loop runs slower, so
           end;                           % use smaller depth (Mandelbrot only)
