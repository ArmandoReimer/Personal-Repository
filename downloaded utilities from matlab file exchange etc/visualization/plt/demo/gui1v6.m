...
% example called gui2.m) by watching the video you can download here:
% www.mennen.org/plt/pltfiles/demoImages/MatlabGUIbuilding.mp4
%
% gui1.m uses a uitable which aren't supported in Matlab 6, so if you are
% running a version of Matlab older than 7.0 then you should run this alternate
% version (gui1v6.m) which replaces the uitable with a radio button. If you
% start gui1 from demoplt, demoplt checks the Matlab version and runs gui1 or
% gui1v6 as appropriate.
%
...
% command that created the object. 
  ...
  uistack(h(1),'bottom');                          % make sure the frame doesn't cover anything
