...
function circles12(in)
  if nargin & ~in
     ...
     while ishandle(cbo) & strcmp(get(cbo,'str'),c)
        s = plt('edit',he);  e = e.*exp(s^3 * .0001i);  m=m+1;
  ...
  plt(cN*r + repmat(c,N,1),'FigName','12 Circles (solution 3)',...
      'LineWidth',2,'Options','Ticks','Pos',[10 650 640 669],...
  ...
  for k = [2 1]
    ...
    plt(cN*r + repmat(c,N,1),'LabelY','','LabelX','','Pos',[600 0],...
       ...
       'Link',fg,'Options','Ticks','TraceID',ID,'LineWidth',2);
  end;
  ...
  h = plt(e,cN,'Pos',[666 919 0 400],'Figname','12 circles (part 2)',...
          'Link',fg,'Options','Ticks Nocur -All','TraceID',['div ';'circ'],...
          'LabelX','','LabelY','','Xlim',[-1.1 1.1],'Ylim',[-1.1 1.1]);
  set(gca,'xtick',[],'ytick',[]);                                    % remove tick marks
  he = plt('edit',[140 8 35 16],[2 0 9],'label','Rotation spe~ed:'); % rotation speed
