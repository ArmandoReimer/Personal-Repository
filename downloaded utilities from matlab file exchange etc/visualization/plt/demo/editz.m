...
% extensively (as in the lower plot of this example) it is useful to
% put it in the "persistent editing mode" which is done on line 131.
% Also, in the following line (line 132) the application data variable
% 'EditCur' is used to change the default size of the cursors used for editing.
%
...
function editz(a)               % A primitive filter design program.
  ...
  % initialize ------------------------------------------------------------------------
  helpT = {...  % help text
   'Only the roots on or above the x axis are shown' .16+.88i 2i ...
   '- To move a root:' ...
   ...
   '- To remove the cursored root, click on the Delete P/Z button' .16+.47i 'color' 505001};
  ...
  plt xright Yedit 8;                                     % use persistent editing mode
  setappdata(gcf,'EditCur',getappdata(gcf,'EditCur')+2);  % make edit cursor larger
  uicontrol('string','new zero',  'units','norm','pos',[.085 .13 .073 .03],'Callback',@newPZ);
  uicontrol('string','new pole',  'units','norm','pos',[.085 .09 .073 .03],'Callback',@newPZ);
  uicontrol('string','delete p/z','units','norm','pos',[.085 .18 .073 .03],'Callback',@curCB,...
            'ButtonDown',{@curCB 0});
  td = findobj(gcf,'user','TraceID');  td = td(1);
  text(-.7,-16,'editz help','color',[0 1 0],'ButtonD','plt helptext on;','parent',td);
