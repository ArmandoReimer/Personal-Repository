...
% So I wrote winplt.m to create a more useable gui for displaying windows.
% winplt displays traces showing the time and frequency domain shapes
% of 31 different FFT windows and also is a tool for designing your own
% windows by adjusting the kernel coefficients with a slider. You can
% also use winplt's command line interface to return the window time
% shapes for use in your Matlab programs.
%
% While working with this application, you may find the IEEE paper on
% Windows for Harmonic Analysis (by Harris) useful. This is the most
% cited reference on FFT windows and includes descriptions of most of
% the windows plotted by winplt. For your convenience, you can get this
% paper from my website (www.mennen.org) in the section called
% "Signal processing papers".
%
% Most treatments of FFT windows are highly mathematical (such as the
% Harris paper). But if you want to understand some of the basic ideas
% without the many pages of mind numbing equations, take a look at the
% this portion of a signal processing talk I gave many years ago. The
% file is called "windowsTalk.pdf" and you can find it on my web site,
% right next to the Harris paper mentioned above. 
%
% Winplt is also a great example of gui programing using plt, however
% if you are just getting started with gui programming I would recommend
% starting first with some of the shorter examples in the plt\demo folder.
%
...
%
% Click on the Help tag (near the lower left corner of the figure) which opens
% the plt help file starting at topic with details about the winplt user interface.
% Navigate to the "Using the plt window" section for instructions on display
% zooming/panning, enabling traces, and using the cursors.

function [out1,out2] = winplt(id,points,pwr,opt)
VerString = 'Ver 03Jan20';
out1 = 0;
out2 = 0;
switch nargin
case 0,  % no arguments: Create the winplt window
  ...
  S.tr = pltinit(0,zeros(1,6),'FigName','winplt (FFT window shapes)',...
    'xy',[0 .143 .093 .787 .798; -1 .006 .723 .077 .217],'Styles','-:-:-:-:',...
    'TIDcback',@TIDbk,'Ylim',ylim,'YlimR',3.51+ylim/40,'Options','-X-Y',...
    'Right',[2 4 6],'User','--','HelpFile','plt.chm::pltfiles/winplt/winplt.htm',...
    'LabelX','Bin number','LabelY',{'Frequency shape (dB)' 'Time shape'},...
    'Pos',[750 550],'DisTrace',[0 0 1 1 1 1],'AxisLink',0,'TRACEc',TRACEc);
  ...
  text(.96,-.08,VerString,'units','norm','color',[.5 .5 .5],'fontsize',8);
