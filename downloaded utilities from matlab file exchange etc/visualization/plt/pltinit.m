...
while k<=Narg
  ...
  if ischar(y)
    ...
    switch sum(y)
      ...
      case 442,      Xlim     = yy;
      case 443,      Ylim     = yy;
      case 557,     if y(1)=='y'  YlimR = yy;  AXr = 1; else cXYax = ctrip(yy); end;
      case 632,    LabelX   = yy;
      case 633,    LabelY   = yy;
...
%
      ...
      case 310,
      case 878,  plt('helptext','set',yy);  helpT = 1;  HelpFileR = 'plt helptext on;';
...
if ~exFig | ~indep setappdata(gcf,'indep',indep); end;
%
...
set(findobj(gcf,'style','push','str','D'),'userdata',cDEFAULT,'tag',cFileS);
CurIDstr  = {@plt 'cursor' CurID};
if indep CurIDstr0 = {@plt 'cursor' 0}; else CurIDstr0 = CurIDstr; end;
set(AXrl,'userdata',CurID);
...
if ~exFig setappdata(FIG,'xymult',[xmult ymult]); end;
...
text(-.02,.45,'','fontsi',fontsz,'horiz','right','buttond','plt click RMS;','userdata',AX);
if length(Xstring)
...
if nID>1 & TRACEid
  ...
  for k=1:nID
    ...
    allb = all(s == ' ');  bln = bln + allb;
    arow = 999*allb - row;
    d = text(col-.93,arow,s);
    ms = {@plt 'click' [Ret1(k) d]};
    set(d,'fontsi',fontsz,'fontw','bol','color',cTRACE(mod(k-1,ndc)+1,:),'buttond',ms);
    setappdata(d,'ty','-');
    if TRACEmk
      mk = line(TRACEmk+col-1,arow+TRACEmk*0,lpl,get(Ret1(k),lpl),'buttond',ms);
      setappdata(d,'mk',mk);
    ...
    if npch(k)
      ...
      set(d,'vis','of');  if length(mk) set(mk,'vis','of'); end;
    end;
...
%
  ...
  for k=1:length(MenuBox)
    if MenuBox(k)
       ...
       if k==1 set(te,'userdata',HelpFile,'tag',HelpFileR); end;
    end;
  end;
...
if ~exFig | length(ucreq) setappdata(FIG,'ucreq',ucreq); end;
...
set(hYlab,'vis',v,'ui',uicontextmenu('callback','plt hideCur;'));
leftC = cPLTbk;
...
set(AX,'color',leftC);
axes(AX);
if     NoCursor   plt('hideCur');
elseif ~BtnGroup  cobj = plt('cursor',CurID,'obj');  set(cobj(7:10),'vis','of');
end;
cidS = CurID;
cidB = CurID;
if nSP
  ...
  for j = 1:nC
    ...
    for k = 1:npj
      if j+k > 2
        ...
        set(axk,'userdata',cidN);   cidS = [cidS cidN];
        if NoCursor plt('hideCur'); end;
        if indep<0
          CurMain = getappdata(0,'CurMain');  Hd = get(CurMain(cidN),'userdata');
...
if exFig
  ...
  for k = [uic txt]  setappdata(k,'ty',' xy'); end;
end;
for k = [axData amb] setappdata(k,'ty',' xy'); end;
setappdata(FIG,'uic',uic);
for k = findobj(txt,'str','','vis','off')'  txt(find(txt==k)) = []; end;
...
set(findobj(gcf,'str','O'),'userdata',LineSmooth);
plt('misc','tidtop');
axes(AX);
