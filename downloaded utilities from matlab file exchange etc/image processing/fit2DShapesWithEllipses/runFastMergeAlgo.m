...
function [EL,BestIClust,BestTotalPerf,NUMEllipses,area,AIC,BIC,minAICBIC,SI,nCompl,hfig1] = runFastMergeAlgo(A,lines,cols,AICBIC_SELECTION)
disp('FAST MERGING');
...
[AIC(length(ELLSET)),BIC(length(ELLSET)),RES_AICBIC,bestAICBIC,SI(length(ELLSET),1:3)] = getAIC_BIC(nCompl,TotalPerf,length(ELLSET),AICBIC_SELECTION,IClust,EL);
...
function [COVERgain,newEL] = checkMergeEll(newEL,IClust,lab1,lab2,EL,BestDTemp)
lines = size(IClust,1);
cols = size(IClust,2);
