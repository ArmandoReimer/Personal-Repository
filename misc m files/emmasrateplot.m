...
% correspondingColors = []; % each row correspond to a point Nx3 matrix
...
for currentDataSet = dataSetsToInclude
    ...
    %Plotting the values 
    h = plot(apPositions,currentInitialSlopes,'.','MarkerSize',15,...
        'Color',allColors(currentDataSet,:));
    %Plotting the error bars
    errorbar(apPositions, currentInitialSlopes,errorEstimations,'.',...
        'Color',allColors(currentDataSet,:));
%     correspondingColors(end+1:end+numberOfParticles,:) = repmat(h.Color,numberOfParticles,1);
    if sum(isnan(currentInitialSlopes)) ~= length(data(currentDataSet).Particles)
        hold on 
        meanInitialSlope = nanmean(currentInitialSlopes);
        
      
        rectanglePosition = [ min(apPositions) meanInitialSlope*0.95 ...
            max(apPositions)-min(apPositions) meanInitialSlope*0.05];
