...
%dog images in each iteration
    ...
    %Load the data
im_stack = {};
for j = 1:11-1 %For a full analysis, i'll run this to length(DTIF)
...
sigma2 = 2.5;
filterSize = 15; 
neighb = 20; %This should work for a first pass and shouldn't fail on sisters.
thr = 90;
dog_stack  = {};
all_frames = {};
for i = 1:11-1 %Will change this to length(DTIF)-1 for full analysis
...
%         imwrite(uint16(dog_stack{i,j}), [OutputFolder1,filesep,dog_name])
        ...
        for k = 1:n_spots
            ...
            if ~isempty(possible_cent)
                ...
                temp_particles = [temp_particles,[inten, cent_x, cent_y, 0]];
                ...
                try
                    ...
                    [x, y] = meshgrid(1:size(snip,1), 1:size(snip,2));
                    ...
                    [f1,res1] = lsqnonlin(gfit1, init_params);
                    figure(2)
                    surf(x, y, gfit1(f1) + double(snip));
                    figure(3)
                    imshow(snip,[])
                    ...
                    init_params = [1000, 10, 5, 10, 5,1000, 10, 5, 10, 5, 1000];
                    [f2,res2] = lsqnonlin(gfit2, init_params);
                    figure(2)
                    surf(x, y, gfit2(f2) + double(snip));
                    if abs(res1-res2) < .1 
                        temp_particles = [temp_particles,[f1(1), f1(2), f1(4), f1(5)]];
                    else
                        temp_particles = [temp_particles,[f2(1), f2(2), f2(4), f2(11)]];
                        temp_particles = [temp_particles,[f1(6), f1(7), f1(9), f1(11)]];
                    end
                catch
                end
            end
